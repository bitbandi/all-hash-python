...
unsigned char *dcrypt_buffer_alloc(void)
{
  return malloc(DCRYPT_DIGEST_LENGTH);
}

void dcrypt(const char *data, size_t data_sz, char *hash_digest, char *hashRet)
{
  ...
  if(!hash_digest)
  {
    hash_digest = (char*)malloc(DCRYPT_DIGEST_LENGTH);
    allocDigest = true;
  }

  sha256_to_str((const uint8_t*)data, data_sz, hashed_nums, (uint8_t*)hash_digest);

  //mix the hashes up, magority of the time takes here
  uint64_t mix_hash_len = mix_hashed_nums(hashed_nums, (const uint8_t*)data, data_sz, &mix_hash, (uint8_t*)hash_digest);

  //apply the final hash to the output
  sha256((const uint8_t*)mix_hash, mix_hash_len, (uint32_t*)hashRet);
